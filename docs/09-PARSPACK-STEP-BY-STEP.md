...
## مرحله ۷ — اتصال تلفن (سیموتل یا Workano)

### اگر از سیموتل استفاده می‌کنی (پیش‌فرض فعلی):
۱. در `.env` بک‌اند: `TELEPHONY_PROVIDER=simotel` و این‌ها را پر کن:
   - `SIMOTEL_WEBHOOK_TOKEN` = یک رمز قوی دلخواه
   - `SIMOTEL_API_BASE` = آدرس سرور سیموتلت (مثلاً `https://pbx.yourcompany.ir/api`)
   - `SIMOTEL_API_TOKEN` = توکن API سیموتل (برای دانلود فایل ضبط)
۲. در پنل سیموتل، برای سه رویداد **IncomingCall / OutgoingCall / CDRQueue** آدرس Webhook را بگذار:
   ```
   https://آدرس-بک‌اند/api/v1/webhooks/simotel
   ```
۳. در همان تنظیمات سیموتل، فیلد `token` بدنه را برابر `SIMOTEL_WEBHOOK_TOKEN` قرار بده.
> جزئیات کامل: [`10-SIMOTEL.md`](10-SIMOTEL.md)

### اگر از Workano استفاده می‌کنی:
`TELEPHONY_PROVIDER=workano` و آدرس Webhook:
```
https://آدرس-بک‌اند/api/v1/webhooks/workano
```
و همان `WORKANO_WEBHOOK_SECRET` را در Workano هم تنظیم کن.
