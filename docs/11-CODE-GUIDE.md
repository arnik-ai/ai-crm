...
### فرانتِ باشگاه (خودپنهان‌شونده)
- `app/club/page.tsx`: صفحه‌ی عمومیِ باشگاه (سطوح + کاتالوگِ پاداش + رتبه‌بندی). `retry:false`؛
  اگر ماژول خاموش/حذف → پیامِ «فعال نشده».
- `LoyaltyCard` (داخلِ `app/students/detail/page.tsx`): کارتِ باشگاهِ **یک دانش‌آموز** —
  سطح/امتیاز، کدِ دعوت، اعمالِ کدِ معرف، تاریخچه‌ی امتیاز (جمع‌شونده)، و **دریافتِ پاداش**
  (جمع‌شونده): کاتالوگ `GET /rewards` + دکمه‌ی «دریافت» → `POST /rewards/{id}/redeem` →
  نمایش/کپیِ کوپن. کفایتِ امتیاز/سطح در بک‌اند سنجیده می‌شود؛ فرانت فقط دکمه را برای امتیازِ
  ناکافی/موجودیِ صفر غیرفعال می‌کند و پیامِ خطای فارسیِ بک‌اند را toast می‌کند. دمو/خاموش →
  کارت اصلاً رندر نمی‌شود.
- `lib/loyalty.ts`: `levelInfo` (برچسب/ایموجی/رنگِ سطوح).
- منوی «باشگاه» در `Sidebar` و بخشِ راهنمای امتیاز در `guide` هم با کوئریِ `/loyalty/levels`
  خودپنهان‌شونده‌اند.
